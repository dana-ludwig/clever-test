# tagger.py uses CLEVER's terminology and note header file to generate concept
# sequences and other annotated textual information for expressing CLEVER rules
# for automatically labeling events documented in clinical text.
#
# input: file paths to the tagging lexicon with word class mappings, the list of
# clinical note headers, target classes for event extraction, maximum snippet
# length, directory path to the clinical corpus, number of workers, output
# folder and size of n-gram context for n-gram feature generation
#
# output: for target mentions detected using a maximum string length, right
# truncated partial string matching, CLEVER's output files include right and
# left n-gram features (context_left.tsv, context_right.tsv), candidate event
# snippets that can be used for additional processing steps such as SNOMED-CT
# concept extraction (discover.tsv), and CLEVER's extraction files (extraction.tsv). 
